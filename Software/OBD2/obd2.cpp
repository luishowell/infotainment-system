...
class obd2{
    ...
    obd2(string comm_port){
        ...
        if (serial_port == -1){
            ...
            bool fixed = false;
            for (int i=0; i<4;i++){ //tries to setup 4 times before quitting
...
int main(){
    obd2 obd("/dev/rfcomm0");

    if (obd.connected){
        //obd.scan_pids();
        obd.print_supported_pids();

        cout<<obd.decoded_cmd("010C")<<"RPM"<<endl; //engine rpm
        cout<<obd.decoded_cmd("010D")<<"Kph"<<endl; //speed
        cout<<obd.decoded_cmd("010F")<<"C"<<endl;   //intake air temp

        close(obd.serial_port);
    }
